...
public class MyProgressBar extends ProgressBar {
     ...
     * 绘制画笔
     */
    private void initPaint() { //绘制指示器的画笔
    ...
    private void obtainAttributes(AttributeSet attrs) {
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.MyProgressBar); //指示器
        indicator_height = (int) typedArray.getDimension(R.styleable.MyProgressBar_indicator_height, indicator_height);
        indicator_color = typedArray.getColor(R.styleable.MyProgressBar_indicator_color, DEFAULT_INDICATOR_COLOR);
        //底部进度条
        back_height = (int) typedArray.getDimension(R.styleable.MyProgressBar_back_line_height, back_height);
        back_color = typedArray.getColor(R.styleable.MyProgressBar_back_line_color, DEFAULT_BACK_LINE_COLOR);
        //顶部进度
        fore_height = (int) typedArray.getDimension(R.styleable.MyProgressBar_fore_line_height, fore_height);
        fore_color = typedArray.getColor(R.styleable.MyProgressBar_fore_line_color, DEFAULT_FORE_LINE_COLOR);
        //回收资源
        typedArray.recycle();
    }
    ...
    protected synchronized void onMeasure(int widthMeasureSpec,
        ...
        progress_width = getMeasuredWidth() - getPaddingRight() - getPaddingLeft();//进度条的宽度
    ...
    private int measureHeight(int measureSpec) {
        ...
        if (specMode == MeasureSpec.EXACTLY) {//精确的测量模式
            ...
            result = (getPaddingTop() + getPaddingBottom() + Math.max(back_height, fore_height)) + indicator_height;
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        float progressPosX = (int) ((line_endX - triangle_width) * progress_with);
        canvas.drawLine(triangle_width, 0, progressPosX + triangle_width, 0, fore_paint);//顶部
        // 绘制三角形
